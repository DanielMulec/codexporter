...
- Windows Codex CLI: validated baseline retained; April 5 refreshed the gates and install-parity evidence, but did not add a fresh live CLI rerun
...
- The April 2-3 macOS and Windows notes above still describe happy-path-only reconfirmations; the April 3 Linux rerun and the April 5 Windows host rerun both refresh deeper edge-case coverage on the current repo state.
...
- That same April 5 Windows host rerun still reproduced two residual Windows runtime caveats on this machine: a long-path Windows project root still fails during export writing on the repo entrypoint, and successful targeted recovery still surfaces raw `\\?\` path spellings in the success message and checkpoint artifact list. See `codexporter-windows-post-refactor-validation-bug-report-2026-04-03.md` for the historical April 3 audit and the per-platform validation docs for the current status.
