...
- the first three rollout stages are now binding in `pyproject.toml`: `mypy` still runs through the same command surface, but it now also blocks explicit `Any`, import-degraded `Any`, and `Any` expressions across the repository
- repo-wide expression-level no-`Any` enforcement is now active; any further hardening is optional extra tightening rather than unfinished baseline work
