...
| US-10 | The skill offers an explicit compact export mode that preserves visible chronology while omitting bulky raw payloads deterministically. | AC-16, AC-17, AC-18, AC-19 | T-44, T-45, T-46, T-47, T-49 | unit, integration, full-flow | all primary v1 targets | compact-mode invocation and content-density story |
| US-11 | Compact and full exports remain part of the same per-session export history. | AC-20 | T-48 | integration, full-flow | all primary v1 targets | shared checkpoint and export-identity story |
...
| REQ-14 | `07_export_data_contract.md`, `15_markdown_rendering_rules.md`, `27_compact_mode_definition.md`, `13_acceptance_criteria.md`, `19_user_side_acceptance_criteria.md` | `$export --compact` is an additive compact render profile that preserves visible chronology, records `Render profile: compact`, omits qualifying bulky raw payloads deterministically, and keeps short qualifying raw diffs verbatim. | T-44, T-45, T-46, T-47, T-49 | unit, integration, full-flow | automated preferred plus manual platform confirmation | all primary v1 targets | compact-mode contract |
| REQ-15 | `09_checkpoint_behavior.md`, `27_compact_mode_definition.md`, `13_acceptance_criteria.md`, `19_user_side_acceptance_criteria.md` | Compact and full exports share the same per-session export identity, numbering model, and checkpoint semantics. | T-48 | integration, full-flow | automated preferred plus manual platform confirmation | all primary v1 targets | render-profile switching must not fork history |
...
- As of March 31, 2026, the compact export profile now has first-class story, acceptance, scenario, and requirement traceability in this matrix rather than being tracked only in narrative notes.
...
- As of March 27, 2026, OpenAI's current Codex docs are inconsistent about native Windows CLI support, so this repository's Windows validated status is grounded in direct repo-controlled evidence rather than in a fully settled upstream platform guarantee.
- Daniel's retrospective March 22, 2026 reports now provide explicit real-user `skill-installer` installation evidence on macOS, Linux, and Windows devices; detailed per-run installer metadata is still thin, but that is no longer a blocker for the v1 validation baseline.
