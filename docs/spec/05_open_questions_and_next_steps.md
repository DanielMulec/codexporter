...
- On April 2-3, 2026, Daniel revalidated the post-Stage-2 macOS happy path from live Codex surfaces: a retained macOS app thread produced a first-run compact export, and a retained macOS CLI thread produced full, incremental, and compact incremental exports from the current repo state. This reconfirmation was happy-path-only and does not replace the March 27 controlled failure-path close-out evidence.
