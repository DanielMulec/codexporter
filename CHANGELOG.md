...
- Added `codexporter-windows-post-refactor-validation-bug-report-2026-04-03.md` to record the April 3, 2026 Windows 11 ARM post-refactor audit, compare it to the still-open March 28, 2026 session-discovery proposal, and document why Windows should not yet be treated as fully revalidated on the current repo revision.
...
- Updated `docs/validation/macos_cli.md`, `docs/spec/22_platform_validation.md`, and `README.md` to record the April 5, 2026 macOS CLI revalidation rerun truthfully: the installed skill now has explicit parity evidence against the repo, a live installed-skill rerun on the retained CLI thread succeeded with an incremental export plus immediate no-new-content follow-up, and isolated CLI-style replays re-confirmed compact-mode behavior together with the required macOS CLI failure-path matrix on the current repo state.
- Updated `docs/validation/windows_cli.md`, `docs/spec/22_platform_validation.md`, `docs/spec/05_open_questions_and_next_steps.md`, and `README.md` to record the fresh April 5, 2026 Windows Codex CLI rerun on the current repo state: the active CLI thread now has direct first, incremental, and compact incremental installed-skill evidence on Codex `0.118.0`, the copied-state Windows CLI edge-case matrix was replayed again from the active thread row, and the remaining Windows path-length caveat is now documented more precisely as a plain-path boundary rather than as a blanket failure of the current CLI-style `\\?\` row.
- Updated `docs/validation/linux_cli.md`, `docs/spec/22_platform_validation.md`, and `README.md` to record the April 5, 2026 Linux rerun on the current repo state: the installed skill now has explicit parity evidence against the repo, the live installed-skill path was revalidated with first-export and compact-incremental runs, the copied-state Linux edge-case matrix was replayed again, and the README automated baseline now reflects the current `42`-test suite.
- Updated `docs/validation/windows_app.md`, `docs/validation/windows_cli.md`, `docs/spec/22_platform_validation.md`, `docs/spec/05_open_questions_and_next_steps.md`, `README.md`, and `codexporter-windows-post-refactor-validation-bug-report-2026-04-03.md` to record the April 5, 2026 Windows host rerun truthfully: the earlier compact `shell_command` regression and stale-install finding are now closed on the current repo state, Windows app is revalidated again on the checklist, the Windows gates were rerun green with `42` tests, and the remaining Windows caveats are now the long-path write failure plus the raw `\\?\` path-display rough edge.
...
- Updated `docs/validation/windows_app.md`, `docs/validation/windows_cli.md`, `docs/spec/22_platform_validation.md`, `docs/spec/05_open_questions_and_next_steps.md`, and `README.md` so the docs now record the April 3, 2026 Windows 11 ARM audit truthfully: Windows app compact-mode validation is re-opened on the current repo state because real app-style `shell_command` traffic still leaks bulky raw file-read output in compact exports, installed-skill parity with the repo is not currently guaranteed on this machine, and long-path Windows write behavior is still an open risk.
- Updated `skills/export/codexporter/compaction.py`, `tests/test_shell_command_compaction.py`, and `docs/spec/27_compact_mode_definition.md` so compact-mode command detection now normalizes both `exec_command` and `shell_command` inputs, covers JSON-backed and plain-string shell-command payloads, and pins the April 3 Windows app regression with a sanitized app-style rollout fixture.
- Updated `docs/validation/macos_app.md`, `docs/spec/22_platform_validation.md`, `docs/spec/05_open_questions_and_next_steps.md`, and `README.md` to record the April 5, 2026 macOS app revalidation rerun truthfully: a live installed-skill compact export on the active Desktop thread succeeded, isolated app-style replays re-confirmed `shell_command` compact behavior and the required macOS failure-path matrix on the current repo state, the maintained macOS-local automated baseline was re-observed at 42 passing `pytest` cases, and the lower-priority `/var` versus `/private/var` path-alias seam is now documented as deferred work behind the higher-priority Windows follow-up.
- Updated `docs/validation/macos_app.md`, `docs/validation/macos_cli.md`, `docs/spec/22_platform_validation.md`, `docs/spec/05_open_questions_and_next_steps.md`, and `README.md` to record the April 11, 2026 post-session-discovery macOS reruns: live app evidence on Codex `0.119.0-alpha.28`, retained live CLI no-new-content confirmation, green macOS-local gates with 45 passing `pytest` tests plus binding `mypy` and `ruff` checks, and a fresh normalized-path isolated macOS matrix replay where both `vscode` and `cli` surfaces passed all checklist rows and re-confirmed stale-index plus stale-SQLite-thread-row targeted-recovery behavior.
